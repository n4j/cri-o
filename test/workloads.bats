...
function check_conmon_fields() {
	...
	echo "check_conmon_fields: passed args - ctr_id=${ctr_id}, cpushares=${cpushares}, cpuset=${cpuset}"
	...
	if [[ "$CONTAINER_CGROUP_MANAGER" == "cgroupfs" ]]; then
		...
		echo "check_conmon_fields: expected cpushares=${cpushares}, found cpushares=${found_cpushares}"
		if [ -z "$cpushares" ]; then
